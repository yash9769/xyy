...
| Product Specification (§8) | No schema exists. Hisaab's product spec is seven separate Markdown files: `apps/household-help-wage-tracker/{PRD,USER_FLOWS,FEATURES,ARCHITECTURE,TEST_PLAN,PRIVACY,MONETIZATION}.md` | **Not implemented as structured data at all.** This is prose, not a `ProductSpec` JSON record — there is no `product_name`/`mvp_features`/`acceptance_criteria` field anyone could query or validate. Human-readable and complete, but not machine-checkable against this document's shape. **Update (Phase 5):** a structured, versioned `ProductSpecification` now exists. Its schema is `schemas/product-specification.schema.json`; the model and validation are in `factory/dashboard/lib/product/` (see its README); it is persisted in `factory/state/product/product-specifications.jsonl`. Hisaab's Markdown spec predates it and was not migrated. |
